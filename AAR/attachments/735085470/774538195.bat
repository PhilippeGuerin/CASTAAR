...
REM RestAPI queries
	...
	REM ---- AAD CONFIG REST -------------
	SET AADtags="%url%AAD/tags"
	SET AADcommonCat="%url%AAD/common-categories"
	REM -----------------------------------------
	
REM run Rest API command
	ECHO 	getting AAD tags .... %AADtags%
	 curl -k -H "Accept: application/json"  -u "%user%" %AADtags% -o %_inputs_folder%CLIENT-tags.json
	ECHO 	getting AAD tags definition .... %AADcommonCat%
	 curl -k -H "Accept: application/json"  -u "%user%" %AADcommonCat% -o %_inputs_folder%CLIENT-common-categories.json
